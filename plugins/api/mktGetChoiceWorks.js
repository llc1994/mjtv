...
// 获取萌娃秀视频
export default function mktGetChoiceWorks(context) {
    // 统一前后端书写
    ...
    return async function(params) {
        ...
        // 数据处理
        if (reply.data && reply.data.msgCd === that.$msgCode.success && _.isObject(reply.data.body) && _.isArray(reply.data.body.allRecList)) {
            reply.data.body.allRecList.forEach(item  => {
                item.userIcon = item.userIcon === '' ? '/default-head-thumbnail.png' : item.userIcon;
            })
            res = reply.data.body.allRecList;
        }
