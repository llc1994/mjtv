...
// 查询商城首页目录
// ------------------------------------------------------------
export default function getChantMallCategoryList(context) {
    // 统一前后端书写
    ...
    return async function(params) {
        ...
        let reply = await axios({
            url: that.$domain + '/mer/mallCategory/getChantMallCategoryList',
            method: 'post',
            data: params,
        });
        // 数据处理
        if (reply.data && reply.data.msgCd === that.$msgCode.success && _.isArray(reply.data.body)) {
            reply.data.body.forEach(function(item, index) {
                let str = [];
                item.smallCategoryList.forEach(function(item2) {
                    str.push(item2.name);
                });
                item.my_name = str.join('/').replace('//', '/');
            });
            res = reply.data.body;
        }
        // 返回结果
