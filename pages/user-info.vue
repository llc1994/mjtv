                    ...
                    <div class="user-mengwa-show">
                        <div class="ta-infos">
                            <div class="head-image">
                                <img :src="heInfos.userIcon || '/default-head-thumbnail.png'" object-fit="cover">
                            </div>
                            <div class="name">开心宝宝：{{heInfos.babyName}}</div>
                            <div class="sign">{{ heInfos.sign || '本宝宝很懒，还没有发布签名哦~' }}</div>
                        </div>
                        <div class="big-title">
                            <!-- <img src="/home.png" /> -->
                            TA的萌娃秀
...
export default {
    ...
    async asyncData(context) {
        ...
        let heInfos = await axios({
            url:context.app.$domain + '/urm/getAssignUserInfo',
            method:'post',
            data:{
                associatedId:context.query.associatedId,
                userId:context.store.state.session.token || ''
            }
        })
        res.heInfos = heInfos.data.body
        // 萌娃秀
        let MengBaoVideo = await axios({
            ...
            data: {
                pageNum: 1,
                pageSize: 20,
                userId: context.query.associatedId
            }
        });
