                ...
                <div class="orders-list">
                    <div class="table">
                        ...
                        <div v-if="orders">
                            ...
                            <div class="tbody fx">
                                <!-- 遍历同一商户下的订单 -->
                                <div class="tr fx" v-for="(item, index) in orders.goodsList" :key="index">
                                    <div class="td flex-percent-30 td-line2" style="padding:10px;">
                                        <div class="good-table-info">
                                            <img :src="item.mallGoodsEspImg" />
                                            <div class="text">
                                                {{ item.mallGoodsName }}
                                            </div>
                                        </div>
                                    ...
                                    <div class="td flex-percent-10">
                                        {{ item.mallGoodsAmount }}
                                    </div>
                                    <div class="td flex-percent-20"></div>
                                    <div class="td flex-percent-10">
                                        {{ (item.skuSaleprice * item.mallGoodsAmount).toFixed(2) }}
                                    </div>
                                    <div class="td flex-percent-10">
                                        {{ orders.expressFee }}
                                    </div>
                            ...
                            <div class="pay-tips">
                                合计金额：<span class="highlight">{{ orders.payTotalPrice }}</span
                                >元
                            </div>
                            <div class="line"></div>
                            <div class="order-text" v-if="orders">
                                <div>
                                    订单编号：<span class="order-text-inner">{{ orders.mallOrderId }}</span>
                                </div>
                                <div>
                                    创建时间：<span class="order-text-inner">{{ orders.createTime }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        // 商品订单详情
        // goodsOrderDetail() {
        //     return MJStore.get('mj_goodsOrderDetail') || {};
        // },
        // // 订单结果
        // goodsOrderResult(){
        //     return MJStore.get('mj_goodsOrderResult') || {};
        // },
            ...
            return (
                {
                    ...
                    REFUND: '退款',
                    CLOSED: '关闭',
                }[this.orders.orderStatus] || this.orders.orderStatus
            );
        },
    },
    components: {
    ...
    mounted() {
        // localStorage.setItem('buyGoodsTimeStart',Date.now());
        this.goodsOrderDetail = MJStore.get('mj_goodsOrderDetail') || {};
        this.goodsOrderResult = MJStore.get('mj_goodsOrderResult') || {};
        this.creatAliQrCode();
        this.creatWxQrCode();
    ...
    methods: {
        ...
        async queryOrderInfo() {
            ...
            let reply = await axios({
                ...
                data: {
                    out_trade_no: this.goodsOrderResult.orderId,
                },
            });
        ...
        async loadOrderInfoByMyCart() {
            ...
            if (reply.data && reply.data.msgCd === that.$msgCode.success && reply.data.body !== undefined && reply.data.body !== null) {
